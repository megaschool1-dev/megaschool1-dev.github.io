...
{
    public static string VideoShareable(ShareableDto video) => VideoShareable(video.ShareableTitle, video.Platform == VideoPlatform.None ? video.Url! : video.MinimalistUrl()!, video.Duration);

    public static string VideoShareable(string heading, string videoUrl, TimeSpan? videoLength)
    {
        var durationEstimate = videoLength == null ? 0 : Util.MinuteEstimate(videoLength.Value).Minutes;
